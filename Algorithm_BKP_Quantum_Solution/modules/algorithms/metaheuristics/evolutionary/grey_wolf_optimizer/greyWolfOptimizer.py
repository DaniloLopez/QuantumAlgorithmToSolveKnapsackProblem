...
class GreyWolf(PopulationMetaheuristic):
        ...
        while t < self.max_efos:
            for it in range(self.pop_size):
                ...
                for i in range (self.pop_size): #TODO revisar n_item
                    if g_best.fitness < Q[i].fitness: # TODO evaluar si se puede evaluar con max()
                        g_best = Q[i]
                        pos_g_best = i

                pos_qr1_qr2_selected = []
                # select position of qr1 and qr2 randomly
                while len(pos_qr1_qr2_selected) < ELITISM_WOLVES - 1: # -1 porque ya que se tiene el alpha
                ...
                if q_c_solution.fitness > Q[it].fitness:
                    Q[it] = q_c_solution
                # else:
                Q[it].quantum_theta += self.__update_by_rgwo(Q.copy(), t, it)
    ...
    def __apply_adaptive_mutation(
        ...
        for i in range (len(list_search_agents[pos_alpha].quantum_theta)):
            q_o_m.append(
                list_search_agents[pos_alpha].quantum_theta[i] + (
                    ft * (
    ...
    def __update_by_rgwo(self, population, t, it):
        ...
        beta = pop[1]
        gama = pop[2]
        angle = THETA_MIN + (1 - t/self.max_efos) * (THETA_MAX- THETA_MIN) #equation 22
        
        for j in range(self.my_knapsack.n_items):
            upsilon_alpha = self.__get_upsilon_wolves(alpha, pop[j], t)
            upsilon_beta = self.__get_upsilon_wolves(beta, pop[j], t)
            upsilon_gama = self.__get_upsilon_wolves(gama, pop[j], t)
            ...
            # TODO vector s con todas las dimensiones

    def __get_upsilon_wolves(self, leader_wolves, actual_wolf, t):
        if leader_wolves.fitness > actual_wolf.fitness:
            return leader_wolves.fitness / actual_wolf.fitness
        else:
            return (np.random.normal(_MU, _SIGMA) * self.max_efos) / (
                K * (self.max_efos + t)
            )
