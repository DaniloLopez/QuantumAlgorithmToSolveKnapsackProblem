            ...
            for folder_name in list_folder_dataset_generated:
                ...
                for file in list_files:
                    ...
                    for it in range(num_iterations):  
                        ...
                        my_metaheuristic.execute(obj_kp, np.random.seed(it)) #invocation execute metaheuristic
                        elapsed_time = time() - start_time #final time
                        list_fitness.append(my_metaheuristic.my_best_solution.fitness)
                        list_efos.append(my_metaheuristic.current_efos)
                        list_times.append(elapsed_time - start_time)
                        times_found_ideal += 1 if ((my_metaheuristic.my_best_solution.fitness - obj_kp.objetive) < 1e-10) else 0                     
                    line_result = util.get_line_result_format(obj_kp, [5], [5], times_found_ideal, num_iterations, times)        
